...
var file = require('./tools/lib/file');
var cstmzPath = path.join(__dirname, 'dist');
var cstmzTmp = path.join(__dirname, '.cstmz-tmp');
var DEFAULTS = {
    name: 'cui',
    dist: cstmzPath,
    tmp: cstmzTmp,
    js: path.join(cstmzTmp, 'js/cui.js'),
    less: path.join(cstmzTmp, 'less/cui.less'),
...
gulp.task('archive:copy', function() {
    return gulp.src(['./dist/' + DEFAULTS.name + '.css', './dist/' + DEFAULTS.name + '.js'])
        .pipe(gulp.dest('./docs/examples/assets'));
});

gulp.task('customizer', function(cb) {
    runSequence(
        'customizer:preparing', ['customizer:less', 'customizer:js'],
        'customizer:clean',
        'archive:copy',
        cb);
});
gulp.task('default', ['customizer']);